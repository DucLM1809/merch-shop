...
- **File layout**: flat — `e2e/pages/`, one `e2e/fixtures.ts`, spec files at `e2e/*.spec.ts` root (no `e2e/tests/` subfolder). Revisit nesting only if the suite grows past ~15 spec files.
